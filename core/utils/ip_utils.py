...
class IpUtils:
    ...
    def save_ipv4s(app_id, ipv4_list):
        for ipv4 in ipv4_list:
            ip_entity = db_session.query(IPv4s).filter_by(app_id=app_id).filter_by(ip=ipv4).first()
    ...
    def save_ipv6s(app_id, ipv6_list):
        for ipv6 in ipv6_list:
            ip_entity = db_session.query(IPv4s).filter_by(app_id=app_id).filter_by(ip=ipv6).first()
            if ip_entity is None:
    ...
    def save_ipblocks(app_id, ipblock_list):
        for ip_block in ipblock_list:
            ipblock_entity = db_session.query(IPv4s).filter_by(app_id=app_id).filter_by(ip_block=ip_block).first()
            if ipblock_entity is None:
